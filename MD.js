...
export class MDUtils {
  ...
  static processLinksItem(line) {
    ...
    line = line.replace(lineTitleResult[0], MDRender.link(lineTitleResult[2], lineTitleResult[5]));
  ...
  static processImagesItem(line, imageClass) {
    ...
    line = line.replace(lineResult[0], MDRender.image(lineResult[6], lineResult[4], imageClass));
    ...
    const processCodeLines = () => {
      ...
      for (let i = 0; i <= iLast; i++) {
        if (i === 0) {
          ...
          if (useLabel) lineTagParts.push(`<span class=${configUI.codeLabel['class']}>${language.toUpperCase()}</span>`);
